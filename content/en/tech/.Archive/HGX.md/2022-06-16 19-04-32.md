...
1. State monitoring. There is usually a role, such as "Oracle", "Validator" or "Relayer", which is responsible for monitoring the state on the source chain.
2. Message passing/forwarding: after this role receives an event, it needs to transfer the message from the source chain to the destination chain.
3. Consensus building: in some models, a consensus is needed between the roles monitoring the source chain in order to pass that information to the destination chain.
...
![桥的信任频谱 来源：Li.Fi](https://tva1.sinaimg.cn/large/e6c9d24egy1h3a7w545irj20v90fn0ul.jpg)
